import type { MatchResult, ProjectInfo, ResolvedParticipant, RosterRecord, ValidationIssue, ValidationTarget } from "../types";
...
import { participantIdentity } from "./participants";
...
export const validateLeaderSubmission = (
  ...
  if (!participants.length) issues.push(issue("participants-none", "参加者が選択されていません", "引継ぎデータを照合するか、全体名簿から追加してください。", { step: 0, fieldId: "participant-matches" }));
  ...
  const identities = participants.map(({ participant }) => participantIdentity(participant)).filter((identity): identity is string => identity !== null);
  if (identities.some((identity, position) => identities.indexOf(identity) !== position)) issues.push(issue("participants-duplicate", "同じ名簿メンバーが重複しています", "重複した参加者選択を解消してください。", { step: 0, fieldId: "participant-matches" }));
  for (const { participant, rosterIndex, addedParticipantId } of participants) {
    for (const field of ["studentId", "name", "faculty", "gender", "address", "phone", "emergencyPhone"] as const) {
      ...
      if (error) issues.push(issue(`participant-${participant.rowId}-${field}`, `${participant.name || "参加者"}の${participantFieldLabel(field)}を確認してください`, error, participantValidationTarget(rosterIndex, participant.rowId, field, addedParticipantId)));
    }
  }
  // A selected organizer is roster-backed. Validate current roster values so an
  // in-project correction is reflected immediately, before autosave completes.
  const rosterOrganizer = project.organizer.rosterIndex === null ? null : roster[project.organizer.rosterIndex] ?? null;
  const organizer = rosterOrganizer ? {
    ...project.organizer,
    studentId: rosterOrganizer.studentId,
    name: rosterOrganizer.name,
    faculty: rosterOrganizer.faculty,
    department: rosterOrganizer.department,
