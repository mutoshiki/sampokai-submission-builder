...
export const validateHikingPlan = (project: ProjectInfo, plan: PlanInfo): ValidationIssue[] => {
  ...
  if (projectMissing.length) issues.push(issue("plan-project-options", "error", "基本情報に未入力があります", `${labels(projectMissing)}。入力後にWordを作成できます。`, projectMissing[0][2]));
  ...
  if (organizerMissing.length) issues.push(issue("plan-organizer-options", "error", "企画者情報に未入力があります", `${labels(organizerMissing)}。入力後にWordを作成できます。`, organizerMissing[0][2]));
  ...
  if (scheduleMissing.length) issues.push(issue("plan-schedule-options", "error", "行程数値に未入力があります", `${labels(scheduleMissing)}。入力後にWordを作成できます。`, scheduleMissing[0][2]));
  ...
  if (safetyMissing.length) issues.push(issue("plan-safety-options", "error", "安全情報に未入力があります", `${labels(safetyMissing)}。入力後にWordを作成できます。`, safetyMissing[0][2]));
  ...
  if (invalidStructure || incomplete.length) {
    ...
    issues.push(issue("plan-itinerary", "error", "行程を確認してください", detail, validationTargets.plan.itinerary));
  }
  if (!plan.policeContacts.some((contact) => contact.label && contact.phone)) issues.push(issue("plan-police-missing", "error", "管轄連絡先が未入力です", "警察署・山岳安全関係の名称と電話番号を追加してください。", validationTargets.plan.policeContacts));
